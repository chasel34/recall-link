...
export function SourceItem({ title, url, item_id, className, ...props }: SourceItemProps) {
  ...
  const containerClasses = cn(
    "group flex items-center gap-3 py-2 px-3 rounded-lg border border-transparent hover:border-border hover:bg-muted/50 transition-all max-w-full text-left",
    className
  )
...
export function SourcesBlock({ children, className, ...props }: React.HTMLAttributes<HTMLElement>) {
  return (
    <span className={cn("block mt-4 pt-3 border-t border-border", className)} {...props}>
