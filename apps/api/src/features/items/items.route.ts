...
const userModelConfigError = {
  error: 'USER_MODEL_CONFIG_MISSING',
  message: 'User AI configuration is missing or invalid.',
} as const

function assertUserModelConfigAvailable(db: ReturnType<typeof getDb>, userId: string) {
  const record = getUserModelConfig(db, userId)
  const apiKeyEnc = record?.api_key_enc?.trim()
  if (!record || !apiKeyEnc) {
    return userModelConfigError
  }
...
itemsApp.post('/', zValidator('json', createItemSchema), (c) => {
  try {
    ...
    if (ai_mode === 'user') {
      const error = assertUserModelConfigAvailable(db, userId)
      if (error) {
