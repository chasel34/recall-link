...
* **批量导入：支持上传 Netscape 格式的浏览器书签 HTML 文件**
* 可选：支持从剪贴板自动识别链接
...
* AI 模式（面向高级用户）：
  ...
  * Server（默认）：服务端统一调用模型（使用环境变量配置的官方 Key）
  * User：用户提供自己的 API Key，由服务端加密存储并代表用户进行调用（当前仅支持 Google Gemini）
    * 需要明确的安全提示：API Key 将加密存储在服务器数据库中，不会存储在浏览器本地
    * 影响范围：
      * Item 分析：抓取完成后由服务端使用用户 Key 生成标签/摘要
      * Chat：服务端使用用户 Key 生成回答，并保持对话记录云端存储
* 默认检索范围：
